...
int copy_directories(int src, char *source, char *dest)
{
    DIR *dir;
    struct dirent *entry;
    ...
    if (copy_file_to_directory(src, source, dest) < 0)
    {
        close(src);
        closedir(dir);
        return EXIT_FAILURE;
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        int new_src = open(new_source, O_WRONLY | O_TRUNC | O_CREAT, 0644);

        if (entry->d_type == DT_DIR && strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
        {
            copy_directories(new_src, new_source, path);
        }
        else
        {
...
int cp(int argc, char **argv)
{
    ...
    /*else if (d_src && d_dst)
    {
        copy_directories(src, source, dest);
    }*/ 
    else
    {
